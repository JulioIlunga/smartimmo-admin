...
//        $this->calendarService = $cs;
    ...
    {
        if ('start' == $type && $reservation->getCheckIn()->getTimestamp() < $date) {
            return true;
        } elseif ('end' == $type && $reservation->getCheckOut()->getTimestamp() > $date) {
            return true;
        } else {
            return false;
        }
...
//                    $reservations[$compareReservationIdx]->getCheckIn()->getTimestamp() == $currentReservation->getCheckOut()->getTimestamp()
//            ) {
//                return false;
//            } else {
//                if ($currentReservation->getCheckOut()->getTimestamp() <= $today + ($period * 3600 * 24)) {
//                    return true;
//                } else {
//                    return false;
//                }
...
//            $compareReservationIdx = $reservationIdx - 1;
//            if (array_key_exists($compareReservationIdx, $reservations) &&
//                    $reservations[$compareReservationIdx]->getCheckOut()->getTimestamp() == $currentReservation->getCheckIn()->getTimestamp()
//            ) {
//                return false;
//            } else {
//                if ($currentReservation->getCheckIn()->getTimestamp() >= $today) {
//                    return true;
//                } else {
//                    return false;
//                }
...
//    }
    ...
    public function isSingleReservationForDayFilter(int $today, int $period, int $reservationIdx, array $reservations, string $type = 'start') : bool
    {
        ...
        if ('end' == $type) {
            ...
            if (array_key_exists($compareReservationIdx, $reservations)
                && $reservations[$compareReservationIdx]->getDateIn()->getTimestamp() == $currentReservation->getDateOut()->getTimestamp()
            ) {
                return false;
                ...
                $end = $this->date2UTC($currentReservation->getDateOut());
            ...
            $compareReservationIdx = $reservationIdx - 1;
            $tmpStart = clone $currentReservation->getDateIn();
            // wenn es eine vorherige reservierung gibt und diese nicht am gleichen tag endet wie die andere startet
            if (array_key_exists($compareReservationIdx, $reservations)
                && $reservations[$compareReservationIdx]->getDateOut()->getTimestamp() == $currentReservation->getDateIn()->getTimestamp()
            ) {
                return false;
                ...
                $start = $this->date2UTC($currentReservation->getDateIn());
...
//    }
    ...
    public function getReservationsForDay(\DateTimeInterface $day, array $reservations): array
    {
        ...
        foreach ($reservations as $reservation) {
            $start = new \DateTimeImmutable($reservation->getCheckIn()->format('Y-m-d').' UTC');
            $end = new \DateTimeImmutable($reservation->getCheckOut()->format('Y-m-d').' UTC');
